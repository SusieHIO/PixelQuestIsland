...
class Game:
    def __init__(self):
        ...
        self.main_questions = [
            ...
            {"question": "What is the name of the magic game that takes place in the  Harry Potter universe?", "options": ["Harry Potter the 10th game", "Magic School", "Hogwarts Legacy"], "correct": 2},
        ...
        # Load and resize the treasure chest image
        self.treasure_image = Image.open("assets/Treasure_chest.png")
    ...
    def resize_elements(self, event=None):
        ...
        if new_width >= 800 and new_height >= 600:
            ...
            if self.treasure_chest is not None:  # Sjekk at skattekisten er synlig
                # Beregn midtpunktet for bredden og ønsket høyde for skattekisten
                chest_x = new_width // 2  # Sørg for at skattekisten plasseres midt i bredden
                chest_y = int(new_height * 0.75)  # Plasserer skattekisten nær bunnen av skjermen
    ...
    def show_end_screen(self, message, show_treasure=False):
        ...
        # Oppdater bakgrunnen til vinnerskjermen
        self.background_image = 'question'  # Sett bakgrunnen til spørsmålsskjermen
        self.canvas.itemconfig(self.background, image=self.question_bg_photo)

        if self.treasure_chest is not None:
            self.canvas.delete(self.treasure_chest)

        if show_treasure:
            window_width = self.window.winfo_width()
            window_height = self.window.winfo_height()

            chest_x = window_width // 2
            chest_y = int(window_height * 0.75)
        ...
        try_again_button.place(relx=0.5, rely=0.40, anchor="center")
        quit_button.place(relx=0.5, rely=0.50, anchor="center")
